...
const CUSTOM_TEXT_FONTS = new Set([
  "Arial",
  "Courier New",
  "Georgia",
  "Times New Roman",
...
const validateOrderCreateInput = (body, files = []) => {
  ...
  const customTextFont = readString(body.customTextFont, "customTextFont", { max: 64 });
  ...
  if (embroideryType === "custom") {
    ...
    if (customOption.text && !CUSTOM_TEXT_FONTS.has(customTextFont)) {
      fail("Выбран неподдерживаемый шрифт", "customTextFont");
    }
  ...
  if (email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
    fail("Некорректный e-mail", "email");
  }
  const preferredContact = readString(body.preferredContact, "preferredContact", { max: 80 });
  const deliveryComment = readString(body.deliveryComment, "deliveryComment", {
    max: 500,
    multiline: true,
  });
  const deliveryCity = readString(body.deliveryCity, "deliveryCity", { max: 120 });
  readString(body.cdekAddressLabel, "cdekAddressLabel", { max: 300 });
  const turnstileToken = readString(body.turnstileToken, "turnstileToken", { max: 2048 });
  ...
  return {
    ...
    petFaceCount,
    customText,
    customTextFont: embroideryType === "custom" && customOption.text ? customTextFont : "",
    customOption,
    comment,
