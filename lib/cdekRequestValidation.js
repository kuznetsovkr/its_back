...
const buildCdekCalculatePayload = (data, packageProfiles = []) => {
  ...
  const matchesPreset = packageProfiles.find((profile) =>
    typeof parcel.width === "number" && parcel.width === profile.width &&
    typeof parcel.height === "number" && parcel.height === profile.height &&
    typeof parcel.length === "number" && parcel.length === profile.length &&
    typeof parcel.weight === "number" && parcel.weight === profile.weight / 1000
  );
  if (!matchesPreset) {
